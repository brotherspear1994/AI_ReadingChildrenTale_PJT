...
def synthesize(model, vocoder, text, sentence, prefix=''):
    ...
    Audio.tools.inv_mel_spec(mel_postnet_torch[0], os.path.join(output_path, '{}_griffin_lim_{}.wav'.format(prefix, sentence)))

    if vocoder is not None:
        if hp.vocoder.lower() == "vocgan":
            utils3.vocgan_infer(mel_postnet_torch, vocoder, path=os.path.join(output_path, '{}_{}_{}.wav'.format(prefix, hp.vocoder, sentence)))   
    
    utils3.plot_data([(mel_postnet_torch[0].detach().cpu().numpy(), f0_output, energy_output)], ['Synthesized Spectrogram'], filename=os.path.join(output_path, '{}_{}.png'.format(prefix, sentence)))
...
if __name__ == "__main__":
    ...
    if hp.vocoder == 'vocgan':
        vocoder = utils3.get_vocgan(ckpt_path=hp.vocoder_pretrained_model_path)
    else:
        vocoder = None   
